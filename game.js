...
clearCanvasBtn.addEventListener('click',clearCtxJet,false);
*/
var noseOfJet;
...
var bgSound = new Audio("back.wav");
var enemyKillSound = new Audio("bullet.wav");
var deathSound= new Audio("death.wav");
var fireSound = new Audio("fire.wav");
var speedInc =0;
var jet1 = new Jet();
var btnStart = new Button(535,745,400,480);
var btnTry = new Button(585,765,415,445);
var btnMenu = new Button(335,515,415,445);
var btnAbout = new Button(45,255,400,480);
var btnPlay = new Button(480,780,390,490);
var btnBack = new Button(10,85,425,460);
...
function moveBg(){
	...
	if(bgDrawX1<=-1600)
	bgDrawX1=1600;
	else if(bgDrawX2<=-1600){bgDrawX2=1600;}
	drawBg();
...
function drawEnd(){
	...
	deathSound.play();
	 ...
	 speedInc=0;
			 ctxHUD.clearRect(0,0,gameWidth,gameHeight);
			 ctxHUD.fillText("Aliens Killed : "+jet1.score,600,30);
	
	 canvasBg.style.cursor="default";
		canvasEnemy.style.cursor="default";
		canvasJet.style.cursor="default";
		canvasHUD.style.cursor="default";
...
Jet.prototype.checkHitEnemy = function(){
	 for(var i=0;i < enemies.length;i++){
		if(this.noseX-10 >=enemies[i].drawX && this.noseX-10<=(enemies[i].drawX + enemies[i].width )&& this.noseY-10>=enemies[i].drawY && this.noseY-10<=(enemies[i].drawY + enemies[i].height))
...
Line.prototype.checkHitEnemy = function(){
	 ...
	 for(var i=0;i < enemies.length;i++){
		if(enemies[i].drawX>=noseOfJet && this.drawX>=enemies[i].drawX )
			{//enemyKillSound.pause();
			 //enemyKillSound=0;
			 //enemyKillSound=new Audio("bullet.wav");
			//	enemyKillSound.play();
...
function mouseClicked(e){
	...
	if(endDrawn){
		...
		if(winDrawn){if(btnPlay.checkClicked()){
		winDrawn=false;
		winSound.pause();
		winSound=0;
	...
	if(aboutDrawn){if(btnBack.checkClicked()){aboutDrawn=false;drawMenu();}}
	}
	}
